...
router.get("/projects/:projectId/test-suites", requireAuth, async (req, res) => {
  ...
  const suites = await db.select().from(testSuitesTable).where(eq(testSuitesTable.projectId, req.params.projectId));
...
router.get("/test-suites/:suiteId", requireAuth, async (req, res) => {
  const [suite] = await db.select().from(testSuitesTable).where(eq(testSuitesTable.id, req.params.suiteId)).limit(1);
  if (!suite) {
...
router.put("/test-suites/:suiteId", requireAuth, async (req, res) => {
  ...
  const [updated] = await db.update(testSuitesTable)
    ...
    .where(eq(testSuitesTable.id, req.params.suiteId))
    .returning();
...
router.delete("/test-suites/:suiteId", requireAuth, async (req, res) => {
  ...
  await db.delete(testSuitesTable).where(eq(testSuitesTable.id, req.params.suiteId));
